...
if (isset($_POST["obj"])) {
    $IdBitacora = InsertarBitacora($_POST["obj"]);
    if ($IdBitacora > 0) {
        $response["response"] = "Se realizo correctamente el registro";
        $response["status"] = "SUCCESS";
        $response["code"] = "200";
        $response["bitacora"] = GetBitacora($IdBitacora);
    } else {
...
function InsertarBitacora($arr){
    ...
    $createItemSQL="INSERT INTO bitacoraconsulta(IdSupervisor,IdEnfDoc,IdClasificacion,IdEmpleado,Diagnostico,
                                Turno,CreationDate,ModifiedAt,IsEnabled) 
                    VALUES(?, ?, ?, ?, ?, ?, NOW(), NOW(), 1);";
    if ($createItem = $conn->prepare($createItemSQL)) {
        $createItem->bind_param("iiiisi", $IdSupervisor,$IdEnfDoc,$IdClasificacion,$IdEmpleado,$Diagnostico,
        $Turno);
