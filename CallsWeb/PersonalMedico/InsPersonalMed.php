...
if(isset($nombre) && isset($apellido_pat) && isset($apellido_mat) && $titulo != "0"){
	if (GetPersMedExists($nombre, $apellido_pat, $apellido_mat, $titulo)) {
		$response["status"] = "ERROR";
        $response["code"] = "500";
        $response["response"] = "Ya existe un un registro con la informacion que esta tratando de enviar..";
        echo json_encode($response);
	}else{
		$createItemSQL="INSERT INTO enfdoctable(Nombre, ApellidoPaterno, ApellidoMaterno, Titulo, CreatedAt) VALUES(?, ?, ?, ?, NOW());";
		if ($createItem = $conn->prepare($createItemSQL)) {
		    $createItem->bind_param("ssss", $nombre, $apellido_pat, $apellido_mat, $titulo );
